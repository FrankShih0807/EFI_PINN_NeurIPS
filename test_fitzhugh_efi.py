...
if __name__ == '__main__':
    ...
    model = PINN_EFI(physics_model=physics_model, physics_loss_weight=0, lr=1e-4, sgld_lr=1e-4, lambda_y=10, lambda_theta=10, hidden_layers=[15, 15, 15])
    
    for key, value in model.__dict__.items():
        print('{}: {}'.format(key, value))
        
        
    ts = torch.linspace(0, physics_model.t_extend, 100)
