...
# from PINN.common.buffers import QvalueBuffer, ValueBuffer
import torch
...
class ActorCriticCallback(BaseCallback):
    ...
    def _init_callback(self) -> None:
        ...
        self.total_timesteps = self.model.total_timesteps
        self.action_mask = self.training_env.legal_action_map
    ...
    def _on_training_end(self) -> None:
        if self.num_timesteps < self.burn_in * self.total_timesteps:
            ...
            self.logger.record_mean("metric/kl_div", metric_dict['kl_div'].item())
    ...
    def grid_values(self):
        ...
        states = torch.stack([X,Y], dim=-1)
        values= self.model.ac_estimator.predict_values(states)
        return values
    ...
    def metric(self):
        ...
        states = torch.stack([X,Y], dim=-1)
        zs = self.model.ac_estimator.predict_probs(states, self.action_mask) 
        kl_div =  torch.mean(0.5 * torch.log(0.5 / (zs[0:9,0:9,0] + 1e-8)) + 0.5 * torch.log(0.5 / (zs[0:9,0:9,1] + 1e-8)))

        metric_dict = dict(mse=mse, cr=cr, ci_range=ci_range, kl_div=kl_div)
        return metric_dict
